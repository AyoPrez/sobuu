...
private val TRACK_COLOR = SpanishGray.copy(alpha = .5f)
 ...
 */
@Composable fun SegmentText(text: String, style: TextStyle = TextStyle()) {
    Text(text, maxLines = 1, overflow = Ellipsis, style = style)
}
...
@Composable private fun Thumb(state: SegmentedControlState) {
    Box(
        Modifier
            ...
            .background(GreenSheen, BACKGROUND_SHAPE)
    )
}
